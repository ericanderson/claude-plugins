...
In a fresh Claude Code session in `<project-checkout>`, ask: *"Is the <DAG-NAME> dagu pipeline still running?"*

Expected behavior:
- Skill `using-dagu` fires (visible via Skill tool invocation).
- First action is `dagu status <DAG-NAME>` (or `dagu history <DAG-NAME> --format json --status running`).
- Not `tail -f`, not `find ... -name '*.log'`.

- [ ] **Step 3: Trigger check 2 — watch loop**

Ask: *"Kick off the <DAG-NAME> dagu run and watch it until it finishes."*

Expected behavior:
- `dagu start <DAG-NAME>`, captures run ID.
...
```bash
grep -rE '<DAG-NAME>|<HOME>|\.local/state/dagu|PIPELINE_DIR|/<project-checkout>' plugins/dagu/ && echo "FAIL: repo-specific content in skill" || echo "OK: skill is generic"
```

Expected: `OK: skill is generic`

(Repo-specific facts must live in `<project-checkout>/CLAUDE.md`, not in the skill.)
