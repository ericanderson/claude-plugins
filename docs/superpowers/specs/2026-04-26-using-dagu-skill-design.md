...
Transcript analysis across 36 conversations in `~/.claude/projects/<project>*` and 15 in a separate sandbox-test project surfaced a consistent set of failure modes when Claude was asked to operate dagu runs:
...
- **Skill metadata triggers correctly.** Manual: in a fresh session in `<project-checkout>`, ask "is the dagu run still going?" and verify the skill fires.
...
- **Watch loop works.** Manual: ask Claude to "kick off the <DAG-NAME> dagu run and watch it." Verify (a) baseline reported, (b) ScheduleWakeup fires, (c) transitions surfaced, (d) terminal summary delivered, (e) any failures drilled into.
...
- **No regression on generic-ness.** Skill body must contain no path under `<project-checkout>`, no DAG name, no project-specific quirk. Self-review checklist before commit.
